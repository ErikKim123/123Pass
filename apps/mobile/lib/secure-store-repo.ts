// Mobile Repository — expo-secure-store backed (each vault row is a separate keychain entry).
// For simplicity in the MVP we store the entire state as one keychain blob. Production should
// shard by item id to avoid the 2KB iOS limit per entry.
...
const KEY_STATE = 'vault_state_v1';
const KEY_SESSION = 'vault_session_v1';
...
async function loadState(): Promise<MobileState> {
  const raw = await SecureStore.getItemAsync(KEY_STATE);
  return raw
    ? (JSON.parse(raw) as MobileState)
    : {
        authUsers: [],
        userRecords: [],
        items: [],
...
async function saveState(state: MobileState): Promise<void> {
  await SecureStore.setItemAsync(KEY_STATE, JSON.stringify(state));
}
...
async function setUid(uid: string | null): Promise<void> {
  if (uid) await SecureStore.setItemAsync(KEY_SESSION, uid);
  else await SecureStore.deleteItemAsync(KEY_SESSION);
}
