// chrome.storage.local backed Repository — same contract as InMemoryRepository.
...
const KEY_STATE = '__123pass_ext_state__';
const KEY_SESSION = '__123pass_ext_session__';
...
async function loadState(): Promise<ExtState> {
  ...
  const result = await chrome.storage.local.get(KEY_STATE);
  return (
    (result[KEY_STATE] as ExtState | undefined) ?? {
      authUsers: [],
      userRecords: [],
      items: [],
      shares: [],
...
async function saveState(state: ExtState): Promise<void> {
...
async function getUid(): Promise<string | null> {
  ...
  return (result[KEY_SESSION] as string | undefined) ?? null;
}

async function setUid(uid: string | null): Promise<void> {
  if (typeof chrome === 'undefined' || !chrome.storage) return;
  if (uid) await chrome.storage.local.set({ [KEY_SESSION]: uid });
  else await chrome.storage.local.remove(KEY_SESSION);
}
